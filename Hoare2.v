...
Proof.
  ...
  apply hoare_while.
  eapply hoare_consequence_pre. apply hoare_asgn.
  unfold bassn, assn_sub, beval.
  intros st [H1 H2].
  apply ble_nat_true in H2. simpl in H2.
  rewrite update_eq. simpl.
  rewrite (parity_ge_2 (st X)); assumption.
  intros st H1; subst; reflexivity.
  unfold bassn, assn_sub, beval.
  intros st [H1 H2].
  SearchAbout [not true].
  apply not_true_is_false in H2.
  apply ble_nat_false in H2. simpl in H2.
  rewrite (parity_lt_2 (st X)) in H1; assumption.
Qed.
(** [] *)
...
Proof.
  unfold is_wp, hoare_triple.
  split.
  intros. inversion H; subst.
  simpl. rewrite update_eq. omega.
  intros P' H st HP.
  remember (H st (update st X (st Y + 1))). clear Heql.
  assert ((X ::= APlus (AId Y) (ANum 1)) / st
      || update st X (st Y + 1)) by
    (apply E_Ass; reflexivity).
  remember (l H0 HP). clear Heql0.
  rewrite update_eq in l0. omega.
Qed.
(** [] *)
...
Proof.
  ...
  intros P' H st HP'.
  remember (H st (update st X (aeval st a))). clear Heqq.
  assert ((X ::= a) / st || update st X (aeval st a)) by
    (apply E_Ass; reflexivity).
  apply q in H0; assumption.
Qed.
(** [] *)
...
Proof.
  ...
  intros. intros st HP x.
  remember (H st (update st X x)). clear Heqq.
  assert ((HAVOC X) / st || update st X x) by apply E_Havoc.
  apply q in H0; assumption.
Qed.
End Himp2.
(** [] *)
...
Theorem fact_fact :
  forall n, n <> 0 -> n * real_fact (n - 1) = real_fact n.
Proof.
  intros.
  induction n.
