...
Proof.
  ...
  inversion H2.
  split.
  intro HP. apply H3. apply H. apply HP.
  intro HR. apply H0. apply H4. apply HR.
Qed.
...
Inductive True : Prop := I : True. (* from library ... *)
(** [] *)
...
(* ref: https://www.labri.fr/perso/casteran/CoqArt/everyday/SRC/class.v *)
...
Proof.
  ...
  intros HC P.
  apply HC.
  assert (not_dist: forall A B : Prop,
               ~ (A \/ B) -> (~ A) /\ (~ B)).
    unfold not.
    split.
    intro HA. apply H. left. apply HA.
    intro HB. apply H. right. apply HB.
  intro H.
  apply not_dist in H.
  inversion H.
  unfold not in H0.
...
Lemma implies_to_or_excluded_middle :
...
Lemma classic_de_morgan_not_and_not :
...
Proof.
  ...
  apply H0.
  split.
  intro HP. apply H1. left. apply HP.
  intro HQ. apply H1. right. apply HQ.
Qed.

Lemma de_morgan_not_and_not_excluded_middle :
