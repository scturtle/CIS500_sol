...
Proof.
  ...
  try reflexivity; try (solve by inversion 2).
  Case "ST_Plus1".
  apply IHHy1 in H2; rewrite H2; reflexivity.
  Case "ST_Plus2".
...
Lemma bool_step_prop3_provable:
  bool_step_prop3.
Proof.
...
Proof.
  ...
  generalize dependent y2.
  induction H; subst.
  intros. inversion H; subst. reflexivity. inversion H4.
  intros. inversion H; subst. reflexivity. inversion H4.
  intros.
  inversion H0; subst.
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_If : forall t1 t1' t2 t3,
      t1 ==> t1' ->
      tif t1 t2 t3 ==> tif t1' t2 t3
  | ST_SameTrue : forall t1,
      tif t1 ttrue ttrue ==> ttrue
  | ST_SameFalse : forall t1,
...
Proof.
  unfold deterministic.
  intro contra.
  remember (contra (tif (tif ttrue ttrue ttrue) tfalse tfalse)
                   (tif ttrue tfalse tfalse)
                   tfalse). clear Heqe contra.
  assert (tif (tif ttrue ttrue ttrue) tfalse tfalse ==> tif ttrue tfalse tfalse) by repeat constructor.
  assert (tif (tif ttrue ttrue ttrue) tfalse tfalse ==> tfalse) by repeat constructor.
  apply e in H. inversion H. assumption.
Qed.
...
(* Yes. [ttrue] and [tfalse] can still not be reduced. *)
Theorem strong_progress : forall t,
  value t \/ (exists t', t ==> t').
Proof.
  intros.
...
(* By making some values reduceable or making some non-values not reduceable. *)
*)
(** [] *)
...
Proof.
  ...
  assumption.
  assert (y = y0).
    remember step_deterministic as HD.
    unfold deterministic in HD.
    apply (HD x y y0); assumption.
...
Lemma multistep_congr_2 : forall t1 t2 t2',
     ...
     P t1 t2 ==>* P t1 t2'.
Proof.
  intros t1 t2 t2' H1 H. multi_cases (induction H) Case.
    Case "multi_refl". apply multi_refl.
...
Proof.
  ...
  step_cases (induction Hs) Case; subst.
  ...
  intros.
  inversion H0; subst.
  apply IHHs in H5. constructor; assumption.
 Qed. 
(** [] *)
...
Proof.
  ...
  Case "multi_refl".
  apply nf_same_as_value in H1.
  inversion H1; subst.
  exists n. split. reflexivity. constructor.
  Case "multi_step".
  remember H1 as H2. clear HeqH2.
  apply IHmulti in H2. inversion H2. inversion H3.
...
Proof.
  ...
  simpl.
  assert (evalF t1 = n1) by (apply IHt1; assumption).
  assert (evalF t2 = n2) by (apply IHt2; assumption).
  subst; reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  step_cases (induction Hy1) Case; intros y2 Hy2.
  Case "ST_PlusConstConst".
  inversion Hy2. reflexivity. inversion H2. inversion H3.
  Case "ST_Plus1".
  inversion Hy2; subst.
  inversion Hy1.
...
Theorem strong_progress_fail : exists t,
  ~ value t /\ ~ exists t', t ==> t'.
Proof.
  exists (P ttrue ttrue).
...
Proof.
  ...
  eapply multi_step. apply CS_Par2. apply CS_SeqStep.
    apply CS_AssStep. apply AS_Plus1. apply AS_Id.
  eapply multi_step. apply CS_Par2. apply CS_SeqStep.
    apply CS_AssStep. rewrite H0. apply AS_Plus.
  eapply multi_step. apply CS_Par2. apply CS_SeqStep.
    apply CS_Ass.
  eapply multi_step. apply CS_Par2. apply CS_SeqFinish.
  fold par_loop.
...
Proof.
  ...
  induction n.
  exists st.
  split. eapply multi_refl. assumption.
  inversion IHn as [st' Hn].
  inversion Hn; clear Hn.
  inversion H1; clear H1.
  eapply ex_intro.
  split.
  apply multi_trans with (par_loop, st'). assumption.
  unfold par_loop at 1.
  eapply multi_step. apply CS_Par2. apply CS_While.
  eapply multi_step. apply CS_Par2. apply CS_IfStep.
    apply BS_Eq1. apply AS_Id.
  ...
  eapply multi_step. apply CS_Par2. apply CS_SeqStep.
    apply CS_AssStep. apply AS_Plus1. apply AS_Id.
  eapply multi_step. apply CS_Par2. apply CS_SeqStep.
    apply CS_AssStep. rewrite H2. apply AS_Plus.
  eapply multi_step. apply CS_Par2. apply CS_SeqStep.
    apply CS_Ass.
  eapply multi_step. apply CS_Par2. apply CS_SeqFinish.
  fold par_loop.
  ...
  rewrite update_neq. assumption. intro c. inversion c.
Qed.
(** [] *)
...
Definition compiler_is_correct_statement : Prop :=
  forall (st : state) (e : aexp),
    multi (stack_step st) (s_compile e, []) ([], [aeval st e]).

Theorem s_compile_trans :
  forall e st stack prog,
    multi (stack_step st) (s_compile e ++ prog, stack)
                          (prog, aeval st e :: stack).
Proof.
    aexp_cases (induction e) Case; try reflexivity.
    Case "ANum".
    intros. simpl. eapply multi_step.
    constructor. eapply multi_refl.
    Case "AId".
    intros. simpl. eapply multi_step.
    constructor. eapply multi_refl.
    Case "APlus".
...
Proof.
  ...
  Case "ANum".
  eapply multi_step. simpl. constructor. eapply multi_refl. 
  Case "AId".
  eapply multi_step. simpl. constructor. eapply multi_refl. 
  Case "APlus".
  simpl.
  eapply multi_trans. apply s_compile_trans.
  eapply multi_trans. apply s_compile_trans.
  eapply multi_step. constructor. eapply multi_refl. 
  Case "AMinus".
  simpl.
