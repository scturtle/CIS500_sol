...
After proving small case, [induction] add induction rule for further proof.
...
Proof.
  ...
  rewrite H.
  rewrite IHn'.
  assert (H2: forall a : bool, a = negb (negb a)).
...
Theorem inc_b2u_comm : forall b : bin,
  S (b2u b) = b2u (succ b).
Proof.
  intro b.
  induction b as [| b' | b'].
  ...
  simpl.
  rewrite <- IHb'.
  assert (H: forall n : nat, S (S (n + n)) = S n + S n).
    destruct n.
      reflexivity.
...
(* TODO: from http://staff.ustc.edu.cn/~bjhua/courses/theory/2014/slides/lec1notes.html *)

Fixpoint u2b (n : nat) : bin :=
  match n with
    | O => Zero
    | S n' => succ (u2b n')
  end.

Theorem u2b_b2u : forall n : nat,
  b2u (u2b n) = n.
Proof.
  intro n.
  induction n as [| n'].
  ...
  Case "n = S n'".
  simpl.
  rewrite <- inc_b2u_comm.
  rewrite IHn'.
  reflexivity.
Qed.

Eval compute in (b2u Zero).
Eval compute in (b2u (Even Zero)).
Eval compute in (b2u (Even (Even Zero))).
...
Theorem succ_succ_bindouble : forall b : bin,
  succ (succ (bindouble b)) = bindouble (succ b).
Proof.
  induction b as [| b' | b'].
  reflexivity. reflexivity. reflexivity.
Qed.

Theorem u2b2: forall n : nat,
  u2b (n + n) = bindouble (u2b n).
Proof.
  induction n as [| n'].
  reflexivity.
  simpl. rewrite <- plus_n_Sm.
  simpl. rewrite IHn'.
  rewrite succ_succ_bindouble.
  reflexivity.
Qed.

Theorem u2bS2 : forall n : nat,
  u2b (S (n + n)) = Odd (u2b n).
Proof.
  induction n as [| n'].
  reflexivity.
...
Theorem inc_u2b_comm : forall n : nat,
  succ (u2b n) = u2b (S n).
Proof.
  intro n.
  reflexivity.
...
Theorem b2u_u2b : forall b : bin,
  u2b (b2u b) = normalize b.
Proof.
  intro b.
  induction b as [| b' | b'].
  Case "b = Zero".
...
(* $Date: 2014-08-23 14:04:58 -0400 (Sat, 23 Aug 2014) $ *)
