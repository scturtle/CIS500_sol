...
Proof.
  ...
  apply IHB2.
  apply g_plus3.
  apply gorgeous__beautiful in IHB1.
  apply IHIHB1 in IHB1.
  apply IHB1.
  apply g_plus5.
  apply gorgeous__beautiful in IHB1.
  apply IHIHB1 in IHB1.
...
Proof.
  ...
  rewrite helper_g_times2.
  apply g_plus3. apply g_plus3. fold plus.
  rewrite <- plus_assoc.
  apply IHgorgeous.
  rewrite helper_g_times2.
  rewrite helper_g_times2.
  apply g_plus5. apply g_plus5. fold plus.
  rewrite <- plus_assoc.
  apply IHgorgeous.
Qed.
(** [] *)
...
(* Only induction with step 2 works. *)
*)
(** [] *)
...
Proof.
  intros.
  apply ev_sum with (n:=n+m) (m:=n+p) in H. (* magic *)
...
Lemma pal_rev : forall X (l : list X), pal (l ++ rev l).
Proof.
  intros.
  induction l as [| x l'].
  simpl. apply pal_nil.
  ...
  apply pal_cc with (l:=l' ++ rev l').
  apply IHl'.
Qed.

Lemma pal__rev : forall X (l : list X), pal l -> l = rev l.
Proof.
  intros.
  induction H. (* NOT inversion! *)
  simpl.
  reflexivity.
  reflexivity.
  simpl.
...
Fixpoint init_last {X:Type} (l : list X) : option (list X * X) :=
  match l with
    | [] => None
    | x :: l' => match init_last l' with
                     | None => Some ([], x)
                     | Some (it,lt) => Some (x :: it, lt)
                 end
  end.

Theorem init_last_works_nil :
  forall X (l : list X), init_last l = None <-> l = [].
Proof.
  split.
  Case "->".
  induction l as [| x l'].
...
Theorem init_last_works :
  forall X (l it : list X) (lt : X),
    init_last l = Some (it, lt) <-> l = it ++ [lt].
Proof.
  admit. (* TODO *)
Qed.

Theorem app_nil : forall X (a : list X), a ++ [] = a.
Proof.
  intros. induction a. reflexivity.
  simpl. rewrite IHa. reflexivity.
Qed.

Theorem rev_app: forall X, forall l1 l2 : list X,
...
Proof.
  ...
  intro l2.
  induction l2.
  reflexivity. simpl. rewrite app_nil. reflexivity.
  intro l2.
  simpl. rewrite IHl1. rewrite snoc_with_append. reflexivity.
Qed.

Theorem rev_append: forall X, forall ll : list X, forall xx : X,
  rev (ll ++ [xx]) = xx :: rev ll.
Proof.
  intros.
  destruct ll eqn:HE.
  reflexivity.
  simpl. rewrite rev_app. rewrite snoc_with_append. reflexivity.
Qed.

Theorem app_eq: forall X,forall xx, forall a b :
  list X, a ++ [xx] = b ++ [xx] -> a = b.
Proof.
  intros X xx a.
  induction a.
  intros.
  destruct b. reflexivity. simpl in H. inversion H.
  destruct b. inversion H2. inversion H2.
  intros.
  destruct b. simpl in H. inversion H.
  destruct a. inversion H2. inversion H2.
  inversion H.
  apply IHa in H2. rewrite H2. reflexivity.
Qed.

Lemma rev__pal : forall X (l : list X),  l = rev l -> pal l.
Proof.
  intros.
  induction l as [| x l']. (* Not work. Should base on length? *)
  Case "l = []".
  apply pal_nil.
  Case "l = x :: l'".
  induction (init_last l') eqn: HL.
  induction x0 as [l'' x'].
  apply init_last_works in HL.
  rewrite HL in H.
  SCase "l' = l'' ++ [x']".
  assert (cons_app: forall xx : X, forall a b : list X,
                      xx :: a ++ b = (xx :: a) ++ b).
    intros. reflexivity.
  rewrite cons_app in H.
  rewrite rev_append in H.
  rewrite <-cons_app in H.
  inversion H.
  rewrite <-app_nil with (a:=rev l'') in H2.
...
| empty' : forall n m : nat,  n < m /\ m < n -> empty m n. 
(** [] *)
...
Lemma R_fact: forall m n o : nat, R m n o <-> m + n = o.
Proof.
  split.
  Case "->".
  intro H.
  ...
  intros. apply c1.
  simpl in *. intros. destruct o. inversion H.
  apply c3. inversion H. rewrite <-H1. apply IHn in H1. apply H1.
  simpl. intros.
...
Inductive subseq  : list nat -> list nat -> Prop :=
  | nil_any: forall l2, subseq [] l2
  | ignore1: forall (x : nat) (l1 l2 : list nat),
      subseq l1 l2 -> subseq l1 (x :: l2)
  | match1: forall (x : nat) (l1 l2 : list nat),
...
Proof.
  ...
  simpl.
  inversion H.
  apply ignore1. apply IHl2. apply H2.
  apply match1. apply IHl1. apply H1.
Qed.

(* TODO: lemma subseq_trans *)
  (* FILL IN HERE *)
(** [] *)
