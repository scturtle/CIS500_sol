...
Theorem andb_rewrite :
  forall (a : bool), a = andb a true.
Proof.
  intro a. destruct a.
  reflexivity. reflexivity. Qed.

Theorem orb_rewrite :
  forall (a : bool), a = orb a false.
Proof.
  intro a. destruct a.
  reflexivity. reflexivity. Qed.
...
Example test_b:    (S (b2u (Even (Odd Zero)))) = b2u (Odd (Odd Zero)).
Proof. reflexivity.  Qed.
...
(*   match n with *)
(*     | O => m *)
(*     | S O => minus (plus'' (S (S O)) m) (S O) *)
(*     | S (S O) => (plus'' O (S (S m))) *)
(*     | S n' => S (plus' n' m) *)
(*   end. *)
(** [] *)

(* $Date: 2013-12-03 05:45:41 -0700 (Tue, 03 Dec 2013) $ *)
