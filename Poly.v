...
Proof.
  ...
  assert (H: forall f' : (X*Y) -> Z, prod_curry f' x y = f' (x, y)).
    intro f'. reflexivity.
  rewrite H.
  reflexivity.
Qed.
...
Proof.
  intros X Y Z f p.
  induction p as [x y].
  assert (H: forall f' : X -> Y -> Z, forall x', forall y',
...
Proof.
  ...
  reflexivity.
  simpl. rewrite <- IHl'.
  assert (H: forall l, map f (snoc l n) = snoc (map f l) (f n)).
    intro l.
    induction l as [| n' l''].
    reflexivity.
...
(* [Y] is the type of result.
   [f] combines x with previous result to get new result. *)
...
(* $Date: 2014-09-09 16:46:50 -0400 (Tue, 09 Sep 2014) $ *)
