...
Theorem bag_truth : forall v : nat, forall s : bag,
...
Proof.
  ...
  reflexivity.
  simpl.
  assert (H: forall h, forall l, rev (snoc l h) = h :: rev l).
    intros h l.
    induction l as [| h' t].
    reflexivity.
    simpl. rewrite IHt.
...
Theorem list_truth : forall n, forall l,
  snoc l n = l ++ (cons n nil).
Proof.
  intros n l.
  induction l as [| n' l'].
  reflexivity.
...
Theorem bag_truth2 : forall n : nat, forall s : bag,
  S (count n s) = count n (sum [n] s).
Proof.
  intros n s.
  induction s as [| n' s'].
