...
Proof.
  ...
  Case "tvar". inversion Ht; subst. solve by inversion.
  Case "tapp". right.
  inversion Ht; subst.
  remember H2; clear Heqh. apply IHt1 in h; destruct h.
  remember H4; clear Heqh. apply IHt2 in h; destruct h.
  SCase "both are value".
  assert (exists x0 t0, t1 = tabs x0 T11 t0) by
    (eapply canonical_forms_fun; eauto).
  destruct H1 as [x0 [t0 Heq]]; subst.
  exists ([x0:=t2]t0). auto.
  SCase "t1 or t2 can step".
  destruct H0. exists (tapp t1 x0). auto.
  destruct H. exists (tapp x0 t2). auto.
  Case "tif". right.
  inversion Ht; subst.
  remember H3; clear Heqh. apply IHt1 in h; destruct h.
  SCase "t1 is value".
  destruct (canonical_forms_bool t1); auto; subst; eauto.
  inversion H as [t1' Hstp]. exists (tif t1' t2 t3). auto.
Qed.
(** [] *)
...
Corollary typable_empty__closed : forall t T, 
    ...
    closed t.
Proof.
  unfold closed. intros t T H x c.
  eapply free_in_context with (T:=T) (Gamma:=empty) in c.
...
[tapp (tabs x TBool ttrue) (tvar y) ==> ttrue] (ST_AppAbs)
[emptty |- ttrue \in TBool]
[emptty |- tapp (tabs x TBool ttrue) (tvar y) |- ?]
cannot decide type for y's type is unknown.
[]
*)
...
Proof.
  ...
  apply (Hnot_val H). apply (Hnf H).
  (* x0 can step, preservation works *)
  apply IHHmulti; auto. apply preservation with x0; auto.
Qed.
...
Proof.
  ...
  Case "T_Abs".
  inversion H0; subst.
  apply IHhas_type in H6. subst. auto.
  Case "T_App".
  inversion H1; subst.
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Progress
      Remains true. [zap] is not value but can step.
...
(** Suppose instead that we add a new term [foo] with the following reduction rules:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
      Become false. [tapp (tabs ...) (tabs ...)].

      - Progress
      Remains true.

      - Preservation
      Become false. [foo] is not well-typed.
...
(** Suppose instead that we remove the rule [ST_App1] from the [step]
    ...
    false, give a counterexample.
      ...
      - Progress
      Becomes false. [app (abs ...) ...] is not value and reduceable.
...
(** Suppose we add the following new rule to the typing
    ...
    false, give a counterexample.
      ...
      - Preservation
      Becomes false. [(\y:(Bool->Bool).\x:Bool.y) (\x:Bool.ttrue)].
...
Inductive substi (s:tm) (x:id) : tm -> tm -> Prop := 
  ...
  | s_tabs2 : forall T t1 y t1',
                x <> y -> substi s x t1 t1' ->
  ...
  | s_nat : forall n, substi s x (tnat n) (tnat n)
  | s_succ : forall t t', substi s x t t' -> substi s x (tsucc t) (tsucc t')
  | s_pred : forall t t', substi s x t t' -> substi s x (tpred t) (tpred t')
  | s_mult : forall t1 t2 t1' t2', substi s x t1 t1' -> substi s x t2 t2' ->
...
Proof.
  ...
  Case "->".
  generalize dependent t'.
  t_cases (induction t) SCase; intros; inversion H; subst; simpl; auto.
  SCase "tvar". destruct (eq_id_dec x i); try subst i; auto.
  SCase "tabs". destruct (eq_id_dec x i); try subst i; auto.
  Case "<-".
  generalize dependent t'.
  t_cases (induction t) SCase;
    intros; inversion H; subst; simpl;
    try rewrite eq_id;
    try (rewrite neq_id by assumption); auto.
  SCase "tapp".
  rewrite (IHt1 t1') by assumption.
  rewrite (IHt2 t2') by assumption. auto.
  SCase "tabs".
  rewrite (IHt t1') by assumption. auto.
  SCase "tsucc".
  rewrite (IHt t'0) by assumption. auto.
  SCase "tpred".
  rewrite (IHt t'0) by assumption. auto.
  SCase "tmult".
  rewrite (IHt1 t1') by assumption.
  ...
  SCase "tif0".
  rewrite (IHt1 t1') by assumption.
  rewrite (IHt2 t2') by assumption.
  rewrite (IHt3 t3') by assumption. auto.
Qed.
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_Ifnot0 : forall n t1 t2,
      n <> 0 ->
      (tif0 (tnat n) t1 t2) ==> t2
  | ST_If : forall t1 t1' t2 t3,
      t1 ==> t1' ->
      (tif0 t1 t2 t3) ==> (tif0 t1' t2 t3)
  | ST_Succ : forall t t', t ==> t' -> tsucc t ==> tsucc t'
  | ST_SuccN : forall n, tsucc (tnat n) ==> tnat (S n)
  | ST_Pred : forall t t', t ==> t' -> tpred t ==> tpred t'
  | ST_Pred0 : tpred (tnat 0) ==> tnat 0
  | ST_PredS : forall n, tpred (tnat (S n)) ==> tnat n
  | ST_Mult1 : forall t1 t2 t1',
                t1 ==> t1' -> tmult t1 t2 ==> tmult t1' t2
  | ST_Mult2 : forall t1 t2 t2',
                value t1 -> t2 ==> t2' -> tmult t1 t2 ==> tmult t1 t2'
...
Tactic Notation "step_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "ST_App2" | Case_aux c "ST_If0" 
  | Case_aux c "ST_Ifnot0" | Case_aux c "ST_If"
  | Case_aux c "ST_Succ" | Case_aux c "ST_SuccN"
  | Case_aux c "ST_Pred" | Case_aux c "ST_Pred0"
  | Case_aux c "ST_PredS" | Case_aux c "ST_Mult1"
  | Case_aux c "ST_Mult2" | Case_aux c "ST_Mult"
  ].
...
Lemma canonical_forms_nat : forall t,
  empty |- t \in TNat ->
  value t -> exists n, t = tnat n.
Proof.
  intros t HT HVal.
  inversion HVal; intros; subst; inversion HT; subst.
  exists n. auto.
Qed.
...
Proof with eauto.
  ...
  Case "T_Mult".
    right.
    destruct IHHt1... inversion H; subst. inversion Ht1.
    destruct IHHt2... inversion H0; subst. inversion Ht2.
    exists (tnat (n * n0))...
    inversion H0. exists (tmult (tnat n) x)...
    inversion H. exists (tmult x t2)...
  Case "T_If0".
    right. destruct IHHt1...
    destruct (canonical_forms_nat t1); auto; subst.
    destruct x.
    exists t2...
    exists t3...
