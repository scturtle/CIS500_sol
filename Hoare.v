...
Proof.
  ...
  unfold hoare_triple.
  intro contra.
  remember (contra (update empty_state X 0)
                   (update (update empty_state X 0) X 1)) as E.
  clear contra HeqE.
  assert ((X ::= APlus (AId X) (ANum 1))
            / update empty_state X 0
            || update (update empty_state X 0) X 1).
    apply E_Ass. simpl. rewrite update_eq. reflexivity.
  apply E in H.
  simpl in H.
...
Proof.
  ...
  rewrite update_eq. rewrite H3. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  rewrite update_eq. rewrite H1. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  eapply hoare_consequence_pre. apply hoare_asgn.
  intros st T. unfold assn_sub. split.
  simpl. rewrite update_neq. rewrite update_eq. reflexivity.
...
Proof.
  ...
  repeat (rewrite update_neq by (intro c; inversion c);
...
Proof.
  ...
  intros st [_ H]. apply ble_nat_true in H. omega.
  eapply hoare_consequence_pre. apply hoare_asgn.
  unfold bassn, assn_sub, update, assert_implies; simpl.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_If1 : forall (st st' : state) (b1 : bexp) (c1 : com),
            IFB b1 THEN c1 ELSE SKIP FI / st || st' ->
...
Proof.
  ...
  eapply hoare_consequence_pre. apply hoare_asgn.
  unfold bassn, assn_sub, update, assert_implies.
  intros st [H1 _]. simpl. assumption.
...
Proof.
  constructor.
  apply E_Seq with (update (update empty_state X 1) Y 1).
  apply E_Seq with (update empty_state X 1); constructor; reflexivity.
...
(* 
  Now [c] is executed at least once, so it can produce effect [Q]!
 
               {{P}} c {{P /\ Q}}
        ------------------------------------------  (hoare_repeat)
        {{P}} REPEAT c UNTIL b END {{P /\ Q /\ b}}
*)

Lemma hoare_repeat : forall P Q b c,
  {{P}} c {{ fun st => P st /\ Q st }} ->
  {{P}} REPEAT c UNTIL b END {{fun st => P st /\ Q st /\ bassn b st}}.
Proof.
  intros P Q b c Hhoare st st' He HP.
  remember (REPEAT c UNTIL b END) as wr eqn:Heqwr.
  ceval_cases (induction He) Case;
    try (inversion Heqwr); subst; clear Heqwr.
  Case "E_Repeat".
  inversion He; subst.
  apply Hhoare in H2. apply H2 in HP.
  remember (WHILE BNot b DO c END) as wcom eqn:Heqwcom.
  ceval_cases (induction H4) SCase;
    try (inversion Heqwcom); subst; clear Heqwcom.
  SCase "E_WhileEnd".
  inversion HP.
  ...
  apply bexp_eval_true. assumption.
  SCase "E_WhileLoop".
  apply IHceval2. apply Hhoare in H4_.
  inversion HP. apply H4_ in H0. apply H0.
  reflexivity. assumption.
  intros. apply IHHe. assumption. assumption.
  intros. apply (Hhoare st0 st'). assumption.
  inversion HP. assumption.
Qed.
